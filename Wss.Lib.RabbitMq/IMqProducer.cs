...
namespace Wss.Lib.RabbitMq
{
    ...
    public interface IProducerBasic
    {
        ...
        void PublishString(string exchange, string routingKey, string mss, bool persistence = true, int secondToLive = 0);
...

    {
        ...
        public ProducerBasic(RabbitMQServer rabbitmqServer)
            : base(rabbitmqServer)
        {
        }
        ...
        public void PublishString(string exchange, string routingKey, string mss, bool persistence = true, int secondToLive = 0)
        {
            if (!string.IsNullOrEmpty(mss))
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        IBasicProperties properties = new BasicProperties();
                        properties.Persistent = persistence;
                        base.Publish(this.GetChannel(), true, properties, UTF8Encoding.UTF8.GetBytes(mss), exchange, routingKey);
                        break;
                    }
                    catch (Exception exception)
                    {
    ...
    //public interface IProducerBasic
    //{
    //    void PublishString(string mss, bool persistence = true, int secondToLive = 0);
    //    void Publish(byte[] mss, bool persistence = true, int secondToLive = 0);
    //    void PublishString(string exchange, string routingKey, string mss, bool persistence = true, int secondToLive = 0);
    ...
    //                    IBasicProperties properties = new BasicProperties();
    //                    properties.Persistent = persistence;
    //                    Publish(this.GetChannel(), true, properties, UTF8Encoding.UTF8.GetBytes(mss), exchange, routingKey);
    //                    break;
    //                }
    //                catch (Exception exception)
    //                {
